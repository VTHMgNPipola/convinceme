 ...
 * @author Patti Schank
 */
public class LinkVector extends Vector<Link> implements Serializable {
    ...
    public LinkVector concatenate(LinkVector lv) {
        ...
        for (Enumeration<Link> e = this.elements(); e.hasMoreElements(); ) {
    ...
    public void setWeights(float w, boolean divWeight) {
        for (Enumeration<Link> e = this.elements(); e.hasMoreElements(); ) {
            (e.nextElement()).setWeights(w, divWeight);
        }
    }
}
