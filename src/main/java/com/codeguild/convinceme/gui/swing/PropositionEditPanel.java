...
public class PropositionEditPanel extends JPanel {
    ...
    public PropositionEditPanel(MainFrame config, Proposition prop, boolean isHyp) {
        ...
        if (!mIsHypothesis) {
            mReliabilitySlider.setBorder(new TitledBorder("Reliability of evidence (1 = low, "
                    + ECHOSimulation.MAX_RELIABILITY
                    + " = high)"));
    ...
    private void loadValues() {
        ...
        if (mProposition.hasRating()) {
            mRatingSlider.setValue(mProposition.getRating());
        }
    ...
    private void saveValues() {
        ...
        mProposition.setRating(mRatingSlider.getValue());
